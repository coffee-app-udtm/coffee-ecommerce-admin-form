...
namespace CoffeeEcommerceAdmin._Form.Product
{
    public partial class FormUpdateProduct : Form
    {
        ...
        public FormUpdateProduct(ProductModel product)
        {
            ...
            this.button_cancel_upload_image.Click += Button_cancel_upload_image_Click;
        ...
        private async void Button_submit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Create product
                ProductModel updatedProduct = await productRequest.UpdateProductAsync(product);
                ...
                button_submit.Text = "Cập nhật sản phẩm";
        ...
        private void Button_cancel_upload_image_Click(object sender, EventArgs e)
        {
            pictureBox1.ImageLocation = currentProduct.image;
        ...
        private async void RenderCategoriesComboBox()
        {
            ...
            comboBox_category.SelectedValue = currentProduct.category_id;
        }

        private async void RenderCurrentProduct()
        {
            // Set product name, category, status, price, description
            textBox_name.Text = currentProduct.name;
            textBox_price.Text = currentProduct.price.ToString("F0");
            textBox_description.Text = currentProduct.description;


            // Combobox status có 2 trạng thái 1 Đang bán và 0 Ngừng bán
            comboBox_status.SelectedIndex = currentProduct.status == 1 ? 0 : 1;

            // Set product image vào pictureBox
            pictureBox1.ImageLocation = currentProduct.image;

            // Request product sizes
            List<ProductSize> productSizes = await productRequest.GetProductSizesAsync(currentProduct.id);
            ...
            foreach (ProductSize productSize in productSizes)
            {
                product_sizes.Add(new ProductSize
                {
                    ...
                    size_price = productSize.size_price
                });

                listBox_product_sizes.Items.Add(productSize.size_name + " - " + Util.FormatVNCurrency(productSize.size_price));
            }

            // Request product toppings and check the checkboxes if the product has toppings
            List<ProductTopping> productToppings = await productRequest.GetProductToppingsAsync(currentProduct.id);

            foreach (ProductTopping productTopping in productToppings)
            {
                for (int i = 0; i < checkedListBox_toppings.Items.Count; i++)
                {
                    // Lấy toppig name từ checkedListBox_toppings
                    string toppingName = checkedListBox_toppings.Items[i].ToString().Split('-')[1].Trim();

                    // Lấy topping id từ checkedListBox_toppings
                    string toppingId = checkedListBox_toppings.Items[i].ToString().Split('-')[0].Trim();

                    if (toppingName.Trim() == productTopping.topping_name.Trim())
                    {
                        checkedListBox_toppings.SetItemChecked(i, true);

                        // Push topping id vào product_toppings
                        product_toppings.Add(int.Parse(toppingId));
                    }
                }
            }
